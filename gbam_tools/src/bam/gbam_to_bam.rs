...
/// Converts a graph-path encoded GBAM file back to BAM.
///
/// Identical to `gbam_to_bam` except it supplies the `VariationGraph` to the
/// reader so that `RawSequence` columns stored with `Codecs::GraphPath` can be
/// decoded back to ASCII bases.
pub fn gbam_to_bam_with_graph(in_path: &str, out_path: &str, gfa_path: &str) {
    ...
    template.set_all();
    let mut reader =
        crate::reader::reader::Reader::new_with_graph(file, template, graph).unwrap();
